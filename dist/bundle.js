...
function attachFileToElement(subject, { files, subjectType, force, window }) {
  let dataTransfer = new window.DataTransfer();
  files.forEach(f => dataTransfer.items.add(f)); // temporary fix for https://github.com/abramenal/cypress-file-upload/issues/320
...
function getFileBlobAsync({ fileName, fileContent, mimeType, encoding, window, lastModified }) {
...
function getFileContent({ filePath, fileContent, fileEncoding }) {
...
var validateOptions = ({ subjectType, force, allowEmpty }) => {
